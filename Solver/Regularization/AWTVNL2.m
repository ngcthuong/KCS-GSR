function [result] = AWTVNL2(Y, lambda, nuy, mu, nLoop, Wx, Wy, nl_grX, nl_grY, par)
...
denoF   = par.denoF;    denoV   = par.denoV;  denoV1    = par.denoV1;
...
while(inner ~= nLoop)
    ...
    if(inner == 1)
        Vk = (Ux*(lambda*dxt*(x-bx) + lambda*(y - by)*dyt + nuy*F ...
               - nuy*W + gamma*(dxt*nl_grX + nl_grY*dyt ) )*transpose(Uy));
        Vk  = Vk./(denoV1);
    else
        Vk  = (Ux*(lambda*dxt*(x-bx) + lambda*(y - by)*dyt + nuy*F ...
                    - nuy*W )*transpose(Uy));
