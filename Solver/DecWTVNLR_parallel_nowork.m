...
% 		res_tex.F   = post_filter(2, res_tex.F, 0.0025, image1);        
        ...
        Fc1         = res_car.F + res_tex.F;   
        
        %% The second cartoon 
        F2          = post_filter(opts.tex.filter_mode, tmp_car, 20, image1);   
        Y_tex       =  Y - R*F2*G;
        ...
        %% the 3rd cartoon 
        F3          = post_filter(opts.tex.filter_mode, tmp_car, 10, image1);   
        [res_tex3]  = SepTV_Recovery(opts.tex3.rec_mode, Y_tex, Wx_tex, Wy_tex, res_tex3.F, ...
                                       dx*res_tex3.F, res_tex3.F*dy, opts.tex3, image1);
        Fc3         = F2 + res_tex3.F;   
        
        F_tmp       = 1/3 * (Fc1 + Fc2 + Fc3); 
        F           = post_filter(opts.tex.filter_mode, F_tmp, 5, image1);   
        %% update measurement
		Y           = Y+Y0-R*F*G; 
        
        %% error 
        errorKPlus = sqrt(sum((F(:) - F2(:)).^2)/sum((F(:).^2))); 
        eachErr(outer) = errorKPlus ;
        eachPSNR(outer) =  psnr(F, image1 );   
        
        %% show psnr 
            % Cartoon only  
            psnr1(outer) = psnr(Fc1, image1); 
            psnr2(outer) = psnr(Fc2, image1); 
            psnr3(outer) = psnr(Fc3, image1); 
            psnr4(outer) = psnr(F, image1); 
            res_car.F    = F2;
            display(['     Fc1: ' num2str(psnr1(outer)) ', Fc2:' num2str(psnr2(outer)) ',Fc3:' num2str(psnr3(outer)) ...
                           'F:' num2str(psnr4(outer))]);
