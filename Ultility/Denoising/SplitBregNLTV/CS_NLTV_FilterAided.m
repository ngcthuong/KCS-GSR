function [u0,energy,relmse,psnr_n]=CS_NLTV_FilterAided(Fp,Fmask,opts)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while (condition)  
 ...
 %    u0=denoising_SBNLTV(v1,tau,wopts,opts.nDenoising,tau./10); 
     ...
     if(mod(n+1, 1) ==0)
     u0 = post_filter(opts.filter_mode, u0, opts.sigma, opts.I);
     end;
     display(['   Iter:' num2str(n+1) ', PSNR:' num2str(psnr(uint8(u0), uint8(opts.I)))]); 
 ...
 if (opts.verbose && mod(n,opts.nWeightupdate)==0) 
     fprintf('\n n=%d,residual std2=%f, relative diff =%f',n,energy(n),relmse(n));
   if isfield(opts,'I')  fprintf(',PSNR(u0) =%f',psnr_n(n));end
 end    
 %condition=(n<opts.nOuter&& energy(n)>opts.bTol&&relmse(n)>opts.xTol);
 condition=(n<opts.nOuter&& energy(n)>opts.bTol); 
end
