...
void rof_isoWeight(num** u, num** weight, num** f, num** x, num** y, num** bx, num** by ,
					 double mu, double lambda, int nGS, int nBreg, int width, int height);
	
	
	/*****************Minimization Methods*****************/
void gs_isoWeight(num** u, num** weight, num** f, num** x, num** y, num** bx, num** by , double mu, double lambda, int width, int height, int iter);
...
void gsSpaceWeight(num** u, num**edge, num** x, num** y, num** bx, num** by, double lambda, int width, int height);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    count = 0;
	do{
		rof_isoWeight(u,edge,f,x,y,bx,by,mu,lambda,1,5,rows,cols);
...
void rof_isoWeight(num** u, num** edge, num** f, num** x, num** y, num** bx, num** by ,
								 double mu, double lambda, int nGS, int nBreg, int width, int height){
		int breg;
		for(breg=0;breg<nBreg;breg++){
				gs_isoWeight(u,edge,f,x,y,bx,by,mu,lambda,width, height,nGS);
...
void gs_isoWeight(num** u, num** edge, num** f, num** x, num** y, num** bx, num** by , double mu, double lambda, int width, int height, int iter){
		int j;
		for(j=0;j<iter;j++){
			gsU(u,f,x,y,bx,by,mu,lambda,width,height);
			
			gsSpaceWeight(u,edge,x,y,bx,by,lambda,width,height);
		}
}
